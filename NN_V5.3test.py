...
#torch.save(a_dict_net,"cnn_noformat.pt")
model_None= Net().float()
model_None.load_state_dict(torch.load('cnn_noformat.pt'))
...
plt.title(r"testing results for different data format (100)", **title_font)
#plt.legend(loc=2,prop =legend_font)
plt.grid(True)
plt.savefig('comparaison_data_formats_otherform100.pdf', bbox_inches='tight')
plt.close(fig)
...
plt.ylabel("Precision in \%", **axis_font)
plt.title(r"testing results for different multiplicative factor(100)", **title_font)
#plt.legend(loc=2,prop =legend_font)
plt.grid(True)
plt.savefig('comparaison_multiply_dark_100.pdf', bbox_inches='tight')
plt.close(fig)
