...
for i in range(len(ls_digit)):
    ...
    total=0.0
    digit=ls_digit[i]
    filename = "To_FPGA_"+str(digit)+".txt"
    filename3 = "To_FPGA_"+str(digit)+"dec.txt"
    filename2 = "cnn"+str(digit)+".pt"
...
for i in range(len(ls_digit2)):
    ...
    filename = "uni_To_FPGA_"+str(digit)+".txt"
    filename3 = "uni_To_FPGA_"+str(digit)+"dec.txt"
    filename2 = "uni_cnn"+str(digit)+".pt"
    a_dict = model_None.state_dict()
...
plt.ylabel(r"Precision in $\%$", **axis_font)
plt.title(r"testing precision results for different data format (test data U.D events)", **title_font)
#plt.legend(loc=2,prop =legend_font)
plt.grid(True)
plt.savefig('comparaison_data_formats_otherform_test_uni.png', bbox_inches='tight')
plt.close(fig)
...
plt.ylabel(r"Precision in $\%$", **axis_font)
plt.title(r"testing precision results different multiplicative factor (test data U.D events) ", **title_font)
#plt.legend(loc=2,prop =legend_font)
plt.grid(True)
plt.savefig('comparaison_multiply_test_dark_uni.png', bbox_inches='tight')
plt.close(fig)
trainloader2 = torch.utils.data.DataLoader(trainloader, batch_size=batch_size,
                                           shuffle=False, num_workers=0)
ls_error=[]
...
for i in range(len(ls_digit)):
    ...
    with torch.no_grad():
        for i,data in enumerate(trainloader2, 0):
            ...
            total += labels.size(0)
            correct += (prediction == labels).sum().item()
        print('Accuracy of the network on the 180000 test events: %d %%' % (100.0 * correct / total))
        ls_accuray.append(100.0 * correct / total)
...
for i in range(len(ls_digit2)):
    ...
    total=0.0
    digit=ls_digit[i]
    filename = "100_To_FPGA_"+str(digit)+".txt"
    filename3 = "100_To_FPGA_"+str(digit)+"dec.txt"
    filename2 = "100_cnn"+str(digit)+".pt"
    model_dicti = Net().float()
    model_dicti.load_state_dict(torch.load(filename2))
    ls_i=[]
    ls_res=[]
...
for i in range(len(ls_digit)):
    ...
    plt.plot(i,ls_accuray[i],'o',label = str(ls_digit[i]))
plt.xticks(positions, ls_digit)
plt.xlabel("BITS", **axis_font)
plt.ylabel(r"Precision in $\%$", **axis_font)
plt.title(r" different data format( train data U.D events)", **title_font)
#plt.legend(loc=2,prop =legend_font)
plt.grid(True)
plt.savefig('comparaison_data_formats_train_uni.png', bbox_inches='tight')
plt.close(fig)
...
plt.xlabel(r"mutlifly factor", **axis_font)
plt.ylabel(r"Precision in $\%$", **axis_font)
plt.title(r" different multiplicative factor(train data U.D events)", **title_font)
#plt.legend(loc=2,prop =legend_font)
plt.grid(True)
plt.savefig('comparaison_multiply_dark_uni_train.png', bbox_inches='tight')
plt.close(fig)
