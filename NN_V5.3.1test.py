...
train_set_100 =torch.from_numpy(np.array(f_100['train_data'],dtype=float)).float()
train_label_100=torch.from_numpy(np.array(f_100['train_label'],dtype=float)).type(torch.LongTensor)
size_data_set= int(len(train_set_100))
for i in range(size_data_set):
    trainloader.append([train_set_100[i],train_label_100[i,1]])
"""
loading data test
"""
...
for i in range(20000):
    testloader.append([test_set_100[i],test_label_100[i,1]]) 
...
learning_rate = 0.0001#exp_decay(epoch)
...
plt.grid(True)
plt.savefig('loss_uni100.png', bbox_inches='tight')
plt.close(fig)
...
plt.legend(loc=2,prop =legend_font)
plt.grid(True)
plt.savefig('prediction_uni100.png', bbox_inches='tight')
plt.close(fig)

a_dict_net = net.state_dict()
torch.save(a_dict_net,"cnn_noformat_uni100.pt")
...
#for i in range(len(ls_digit)):
    ...
    #total=0.0
    #digit=ls_digit[i]
    #filename = "To_FPGA_uni100_"+str(digit)+".txt"
    #filename3 = "To_FPGA_uni100_"+str(digit)+"dec.txt"
    #filename2 = "cnn_uni100"+str(digit)+".pt"
...
#plt.title(r"testing results for different data format", **title_font)
##plt.legend(loc=2,prop =legend_font)
#plt.grid(True)
#plt.savefig('comparaison_data_formats_otherform_uni100.pdf', bbox_inches='tight')
#plt.close(fig)
...
##plt.legend(loc=2,prop =legend_font)
#plt.grid(True)
#plt.savefig('comparaison_multiply_uni100.pdf', bbox_inches='tight')
#plt.close(fig)
