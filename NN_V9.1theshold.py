...
def threshold(x,thesh1):
    size = x.size()
    output = torch.empty(size[0],2)
    signal = np.array([1,0])
    background = np.array([0,1])
    for i in range(size[0]):
        if x[i,0]>thesh1:
            output[i] = torch.from_numpy(signal)
        else:
            output[i] = torch.from_numpy(background)
    return output
...
f_100 = h5py.File('data/dark_100.h5', 'r')
...
for i in range(size_data_set):
    trainloader.append([train_set[i],train_label[i,0]])
...
for i in range(20000):
    testloader.append([test_set[i],test_label[i,0]]) 

testloader2 = torch.utils.data.DataLoader(testloader, batch_size=batch_size,
                                          shuffle=False, num_workers=0)
test_set_100 = torch.from_numpy(np.array(f_100['test_data'],dtype=float)).float()
test_label_100 = torch.from_numpy(np.array(f_100['test_label'],dtype=float)).type(torch.LongTensor)
testloader_100=[]

for i in range(20000):
    testloader_100.append([test_set_100[i],test_label_100[i,0]]) 

testloader2_100 = torch.utils.data.DataLoader(testloader_100, batch_size=batch_size,
                                          shuffle=False, num_workers=0)
k=0
n_epoch = 50
ls_batch=[]
...
#plt.grid(True)
#plt.savefig('loss_100.png', bbox_inches='tight')
#plt.close(fig)
...
#plt.legend(loc=2,prop =legend_font)
#plt.grid(True)
#plt.savefig('prediction_100.png', bbox_inches='tight')
#plt.close(fig)

#a_dict_net = net.state_dict()
#torch.save(a_dict_net,"cnn_noformat_100.pt")
model_None= Net().float()
model_None.load_state_dict(torch.load('cnn_noformat_uni100.pt'))
...
ls_tresh = np.linspace(0,0.5,num = 11)
for tr in ls_tresh:
    with torch.no_grad():
        ls_i=[]
        correct=0.0
        total=0.0
        tensors = torch.empty(1,2)
        prediction_tot =  torch.empty(1,1)
        for i,data in enumerate(testloader2, 0):
            images, labels = data
            outputs = model_None(images)
            l =outputs.size()
            tensors=torch.cat((tensors,outputs.data),0)
            prediction = threshold(outputs.data,tr)
            #prediction_tot= torch.cat((prediction_tot,prediction),0)
            results, predictions = torch.max(outputs.data, 1) #if the value predicted is a event return 1 into prediction varibale.
            for i in range(500):
                if labels[i] == 0:
                    total += 1
        ...
        print('Accuracy of the network on the 20000 events: %d %%' % (100.0 * correct / total))
...
for tr in ls_tresh:
    with torch.no_grad():
        ...
        tensors = torch.empty(1,2)
        prediction_tot =  torch.empty(1,1)
        for i,data in enumerate(testloader2_100, 0):
            images, labels = data
            outputs = model_None(images)
            l =outputs.size()
            tensors=torch.cat((tensors,outputs.data),0)
            prediction = threshold(outputs.data,tr)
            #prediction_tot= torch.cat((prediction_tot,prediction[:,0]),0)
            results, predictions = torch.max(outputs.data, 1) #if the value predicted is a event return 1 into prediction varibale.
            for i in range(500):
                if labels[i] == 1:
                    total += 1
                    if prediction[i,0] == 1:
                        correct += 1
        print('Accuracy of the network on the 20000 events 100keV: %d %%' % (100.0 * correct / total))
        ls_accuray_100.append(100.0 * correct / total)
...
#a = tensors[:,0].numpy()
#print(a)
#plt.hist(a.astype('float'),bins =100)
#plt.xlabel(r"", **axis_font)
#plt.ylabel(r"", **axis_font)
#plt.title(r"output[:,0]", **title_font)
##plt.legend(loc=2,prop =legend_font)
#plt.grid(True)
#plt.savefig('plot_dist_output_eventsuni100.png', bbox_inches='tight')
#plt.close(fig)
#fig = plt.figure()

#c = prediction_tot[:,0].numpy()

#fig = plt.figure()
#b = tensors[:,1].numpy()
#plt.hist(b.astype('float'),bins =100)
#plt.xlabel(r"", **axis_font)
#plt.ylabel(r"", **axis_font)
#plt.title(r"", **title_font)
##plt.legend(loc=2,prop =legend_font)
#plt.grid(True)
#plt.savefig('plot_dist_output_noiseuni100.png', bbox_inches='tight')
#plt.close(fig)

fig = plt.figure()
positions=[]
for i in range(len(ls_tresh)):
    positions.append(ls_tresh[i])
    plt.plot(ls_tresh[i],ls_accuray[i],'bo',label = str(ls_tresh[i]))
    plt.plot(ls_tresh[i],ls_accuray_100[i],'r*',label = str(ls_tresh[i]))

plt.xlabel(r"treshold level", **axis_font)
plt.ylabel("Precision in \%", **axis_font)
plt.title(r"testing presion for positive and noise event in function of the treshold", **title_font)
#plt.legend(loc=2,prop =legend_font)
plt.grid(True)
plt.savefig('comparaison_event_treshoduni100.pdf', bbox_inches='tight')
plt.close(fig)
