...
plt.grid(True)
plt.savefig('loss_uni1002.png', bbox_inches='tight')
plt.close(fig)
...
plt.legend(loc=2,prop =legend_font)
plt.grid(True)
plt.savefig('prediction_uni1002.png', bbox_inches='tight')
plt.close(fig)

a_dict_net = net.state_dict()
torch.save(a_dict_net,"cnn_noformat_uni1002.pt")
model_None= Net().float()
model_None.load_state_dict(torch.load('cnn_noformat_uni1002.pt'))
